...
static void event_queue_init(struct event_queue *q) {
  q->head = 0;
  q->tail = 0;
}

static bool event_queue_push(struct event_queue *q, struct platform_event *e) {
...
static bool event_queue_pop(struct event_queue *q, struct platform_event *out) {
  if (q->head == q->tail) {
    return false; /* Empty */
  }
  *out = q->events[q->head];
...
static bool create_buffer(struct platform *p, struct buffer_entry *buf,
...
static void destroy_buffer(struct buffer_entry *buf) {
  if (buf->wl_buffer) {
...
static struct buffer_entry *get_free_buffer(struct platform *p) {
  int i;
...
static void keyboard_keymap(void *data, struct wl_keyboard *keyboard,
  ...
  p->xkb_keymap = xkb_keymap_new_from_string(p->xkb_context, map_shm,
 ...
 */
static uint32_t utf8_to_codepoint(const char *utf8, int len) {
  unsigned char c;
...
static void keyboard_key(void *data, struct wl_keyboard *keyboard,
  ...
  if (state == WL_KEYBOARD_KEY_STATE_PRESSED) {
    utf8_len =
        xkb_state_key_get_utf8(p->xkb_state, keycode, utf8, sizeof(utf8));
    if (utf8_len > 0) {
...
static void keyboard_modifiers(void *data, struct wl_keyboard *keyboard,
  ...
  xkb_state_update_mask(p->xkb_state, mods_depressed, mods_latched, mods_locked,
                        0, 0, group);
  ...
  if (xkb_state_mod_name_is_active(p->xkb_state, XKB_MOD_NAME_LOGO,
                                   XKB_STATE_MODS_EFFECTIVE)) {
...
static void registry_global(void *data, struct wl_registry *registry,
  ...
  if (strcmp(interface, wl_shm_interface.name) == 0) {
    ...
    p->seat = wl_registry_bind(registry, name, &wl_seat_interface, 7);
...
bool platform_should_close(struct platform *p) { return p->closed; }

int platform_get_width(struct platform *p) { return p->width; }

int platform_get_height(struct platform *p) { return p->height; }

bool platform_has_focus(struct platform *p) { return p->has_focus; }
...
uint32_t platform_get_modifiers(struct platform *p) { return p->modifiers; }

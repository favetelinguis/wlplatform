...
struct syntax_node {
	...
	str text; /* View into source (valid while source lives) */
	uint32_t start_row;
	uint32_t start_col;
	uint32_t end_row;
	uint32_t end_col;
...
bool syntax_parse(struct syntax_ctx *ctx, str source);
...
str syntax_node_text(struct syntax_node *node, str source);
